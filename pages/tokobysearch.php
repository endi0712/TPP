         ...
         <div class="form-group">
            <select class="custom-select" name="search">
              ...
              while($r=mysqli_fetch_array($q)){ 
               ?>
              <option value="<?php echo $r['id_pemilik_ukm'] ?>" <?php if($r['id_pemilik_ukm']==$key){echo "selected";} ?>><?php echo $r['nama']; ?></option>
            <?php } ?>
            </select>
  ...
  <section class="ftco-section bg-light">
   <div class="container">
    ...
    <?php 
     ...
     $result = mysqli_query($koneksi,"SELECT * FROM tbl_pemilik_ukm where id_pemilik_ukm = '$key'");
     $total = mysqli_num_rows($result);
     $pages = ceil($total/$halaman);  
     $q = mysqli_query($koneksi, "SELECT * FROM tbl_pemilik_ukm where id_pemilik_ukm = '$key' order by id_pemilik_ukm desc limit $mulai, $halaman");
     $row = mysqli_num_rows($q);
     if ($row==0) {
       ?>
       <div class="col-md-12 d-flex align-self-stretch px-4 mb-5">
        <h4>Maaf Toko tidak ditemukan..</h4>
      </div>
      <?php 
     }else{
     while($r=mysqli_fetch_array($q)){ ?>
      ...
      <div class="col-md-4 d-flex align-self-stretch px-4 mb-5">
        <div class="d-block services text-center">
          ...
          <div class="media-body p-4 klinik">
